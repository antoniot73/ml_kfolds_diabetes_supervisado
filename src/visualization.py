"""Visualización de resultados del pipeline K-Folds."""
...
def plot_confusion_matrix_from_predictions(
    ...
    ax.set_title(f"Matriz de confusión K-Folds - {model_name}")
    plt.tight_layout()
    plt.savefig(output_path, dpi=160)
    plt.close()
...
def plot_metrics_comparison(metrics_df: pd.DataFrame, output_path: Path) -> None:
    ...
    ax.set_title("Comparación de métricas K-Folds por modelo")
