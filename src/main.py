"""Ejecución completa del proyecto K-Folds Diabetes.

Uso desde la raíz del proyecto:
    python src/main.py

El flujo replica la arquitectura de la plantilla base:
datos → validación → transformación → modelado K-Folds → evaluación →
...
from modeling import build_models, evaluate_predictions, run_kfolds_predictions
...
def run_pipeline() -> None:
    """Ejecuta el pipeline completo de análisis supervisado con K-Folds.
    ...
    LOGGER.info("Iniciando pipeline K-Folds Diabetes.")
    ...
    predictions = run_kfolds_predictions(models=models, X=X, y=y, n_splits=10, random_state=42, n_jobs=1)
    metrics_df = evaluate_predictions(predictions)

    save_table(predictions, paths["tables"] / "predicciones_kfolds_cache.csv")
    save_table(metrics_df, paths["tables"] / "metricas_kfolds_modelos.csv")
    ...
    for model_name in models:
        ...
        plot_confusion_matrix_from_predictions(
            ...
            output_path=paths["figures"] / f"matriz_confusion_kfolds_{file_name}.png",
        )

    plot_metrics_comparison(metrics_df, paths["figures"] / "comparacion_metricas_kfolds.png")
    ...
    plot_random_forest_feature_importance(
        importance_df,
        paths["figures"] / "importancia_variables_kfolds_random_forest.png",
    )

    LOGGER.info("Pipeline K-Folds Diabetes finalizado correctamente.")
